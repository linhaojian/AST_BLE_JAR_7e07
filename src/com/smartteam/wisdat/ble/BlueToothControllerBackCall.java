...
import com.example.ast_ble_jar.MainActivity;
...
//			mBleStauscallback.Disconnect(gatt, status);
//		}
		mBleStauscallback.onCharacteristicWrite();
	...
	public void onConnectionStateChange(BluetoothGatt gatt, int status,
		...
		if(status==BluetoothGatt.GATT_SUCCESS&&newState==BluetoothProfile.STATE_CONNECTED){//初步连接成功
			//支持服务
			Log.e("linhaojian","支持服务："+gatt.discoverServices());
		}else{
			Log.e("linhaojian", "断开"+status);
			if(gatt!=null){
				gatt.close();
			}
			if(this.gatt!=null){
...
//		if(descriptor.getCharacteristic().getUuid().toString().equals(uuidStr)){
//			  // 获取服务项
//			BluetoothGattService s = gatt.getService(UUID.fromString(SERI_UUID));
//			// 获取特征--noti/msg
//			BluetoothGattCharacteristic c_noti = s.getCharacteristic(UUID.fromString(MainActivity.INDICATE_CHARAC_UUID));
//			//使能
//			if(gatt.setCharacteristicNotification(c_noti, true)){
//				BluetoothGattDescriptor config = c_noti.getDescriptor(UUID.fromString(MainActivity.INDICATE_DESC_UUID));
...
//				Log.e("linhaojian", "使能："+gatt.writeDescriptor(config));
//			}
//		}else if(descriptor.getCharacteristic().getUuid().toString().equals(MainActivity.INDICATE_CHARAC_UUID)){
			// 描述写入接口
			BluetoothGattService s = gatt.getService(UUID.fromString(SERI_UUID));
			// 获取特征--noti/msg
	...
	public void onServicesDiscovered(BluetoothGatt gatt, int status) {
		...
		if(status==BluetoothGatt.GATT_SUCCESS){
			if(!TextUtils.isEmpty(SERI_UUID)&&!TextUtils.isEmpty(CHAR_NOTI_UUID)&&!TextUtils.isEmpty(CHAR_MSG_UUID)){
				...
				Log.i("BlueToothControllerBackCall", "不需要使能的连接成功！");
				// 获取服务项
				BluetoothGattService s = gatt.getService(UUID.fromString(SERI_UUID));
				// 获取特征--noti/msg
				BluetoothGattCharacteristic c_msg = s.getCharacteristic(UUID.fromString(CHAR_MSG_UUID));
